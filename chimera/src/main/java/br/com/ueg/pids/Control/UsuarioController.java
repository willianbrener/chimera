...
public class UsuarioController extends GenericController<Usuario> {
	...
	public Return validar(IModel<?> imodel) throws SQLException {
		...
		if (getUsuario().getNome() == null || getUsuario().getNome().equals("")) {
			...
			ret = new Return(false, "Nome em branco ou inv�lida!",
					TypeMessage.ERROR);
			...
			ret = new Return(false, "Conta em branco ou inv�lida!",
					TypeMessage.ERROR);
			...
			ret = new Return(false, "Senha em branco ou inv�lido!",
					TypeMessage.ERROR);
		}else if(!mail.validatorEmail(getUsuario().getEmail())){
			ret = new Return(false, "Email em branco ou inv�lido!",
					TypeMessage.ERROR);
		}else if(getUsuario().getPermissao() == null || getUsuario().getPermissao().equals("")){
			ret = new Return(false, "Permissao em branco ou inv�lido!",
					TypeMessage.ERROR);
		}else if(getUsuario().getCargo() == null || getUsuario().getCargo().equals("")){
			ret = new Return(false, "Cargo em branco ou inv�lido!",
					TypeMessage.ERROR);
